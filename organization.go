...
type Organizations interface {
	...
	// Queue shows the current run queue of an organization.
	Queue(ctx context.Context, organization string, options QueueOptions) (*Queue, error)
}
...
// Queue shows the current run queue of an organization.
func (s *organizations) Queue(ctx context.Context, organization string, options QueueOptions) (*Queue, error) {
	if !validStringID(&organization) {
		return nil, errors.New("Invalid value for organization")
	}
